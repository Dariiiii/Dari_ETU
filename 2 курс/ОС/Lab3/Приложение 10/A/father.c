...
int main (void) {
    ...
    int i, pid, pid1, pid2, ppid, status;
    int n;
    ...
    {           // Определяем величину кванта через дочерний процесс
        if (sched_rr_get_interval (pid1, &qp) == 0)
            printf ("SON: Квант процессорного времени RR: %g сек\n",
            qp.tv_sec + qp.tv_nsec / 1000000000.0);
        execl("son", "son", NULL);
    }
    // Снова изменяем приоритет и политику планирования
    if (sched_setscheduler (0, SCHED_FIFO, &shdprm) == -1)
        perror ("SCHED_SETSCHEDULER_");

        // Определяем величину кванта при SCHED_RR
    if (sched_rr_get_interval (0, &qp) == 0)
        printf ("Квант при FIFO планировании: %g сек\n",
        qp.tv_sec + qp.tv_nsec / 1000000000.0);
    else
        perror ("SCHED_FIFO_GET_INTERVAL");

    if((pid2=fork()) == 0)
    {           // Определяем величину кванта через дочерний процесс
        if (sched_rr_get_interval (pid2, &qp) == 0)
            printf ("SON: Квант процессорного времени FIFO: %g сек\n",
            qp.tv_sec + qp.tv_nsec / 1000000000.0);
        execl("son", "son", NULL);
    }
